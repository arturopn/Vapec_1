package com.pepsico.vapec.db
{
	...
	public function start():void
	{
		...
		var db:File = File.userDirectory.resolvePath(ConfigGlobal.APP_NATIVE_PATH + "\\test.db");
		trace(db.nativePath);
	...
	private function addItem():void
	{
		...
		sqls.text = "INSERT INTO test_table (first_name, last_name) VALUES('sd','asda');";
		sqls.execute();
	...
	// function to add item to our database
	public function insertLineToTable(tableName:String, tableColumns:String, rowValues:String):void
	{
		var arrCols:Array = tableColumns.split(",");
		var arrVals:Array = rowValues.split("|");
		
		var columns:String = "";
		var values:String  = "";
		
 		for (var x:int=0; x<arrCols.length; x++)
		{
			columns += arrCols[x];
			values  += arrVals[x];
			
			if(x != arrCols.length-1)
			{
				columns += ", ";
				values  += ", ";
			}
		...
		sqls.text = "INSERT INTO " + tableName + " (" + columns + ") VALUES(" + values + ");";
		trace(sqls.text);
	...
	private function remove():void
	{
		...
		//sqls.text = "DELETE FROM test_table WHERE id="+dp[dp.selectedIndex].id;
		//sqls.execute();
		refresh();
	}
